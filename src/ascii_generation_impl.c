...
const int colorChannelValues[7] = {0, 95, 135, 175, 215, 255};
...
unsigned char *halfHeight(unsigned char *data, int width, int height, int channels) {
  unsigned char *data2 = malloc((width * floor((float)height / 2) * channels) * sizeof(unsigned char));
...
char *generateAsciiArt(unsigned char *imageData, int width, int height, int channels) {
  ...
  unsigned char *temp = halfHeight(imageData, width, height, channels);
  free(imageData);
  imageData = temp;
  height = floor((float)height / 2);
  size_t output_size = (size_t)width * height * (32 + 1 + 1) + 1;

  char *asciiArt = (char *)malloc(output_size);
  asciiArt[0] = '\0';

  char *current_pos = asciiArt;

  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      int pixel = (y * width + x) * channels;

      unsigned char r = imageData[pixel + 0];
      unsigned char g = imageData[pixel + 1];
      unsigned char b = imageData[pixel + 2];

      float brightness = (0.299 * r + 0.587 * g + 0.114 * b);

      unsigned char index = floor((brightness / 255) * (arrayLength - 1));

      int color = getAnsiiColor(r, g, b);
      current_pos += sprintf(current_pos, "\033[48;5;0m\033[38;5;%dm%c\033[0m", color, asciiLetters[index]);
    }
